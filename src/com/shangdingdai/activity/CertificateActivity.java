...
public class CertificateActivity extends BaseActivity implements
		OnClickListener {
	...
	public static final String USETINFO_URL = Constants.SERVICE_ADDRESS+"myinfo/getUserInfo";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (!NetworkUtils.isNetworkAvailable(getApplicationContext())) {
			...
			layout_smrz.setClickable(false);;
			layout_bdsj.setClickable(false);
			layout_rzyx.setClickable(false);
		} else {
			mt = new MyAsyncTask();
			mt.execute(userid);
		}
	...
	private void initDatas() {
		...
		isSmrz = SharePreferenceUtil.getInstance(getApplicationContext())
				.isSmrz();
		isPhone = SharePreferenceUtil.getInstance(getApplicationContext())
				.isphone();
		isRzyx = SharePreferenceUtil.getInstance(getApplicationContext())
				.isRzyx();
		if (isSmrz) {
		...
		if (isPhone) {
			...
			String tel = SharePreferenceUtil.getInstance(
					getApplicationContext()).getUserTel();
		...
		if (isRzyx) {
			...
			String email = SharePreferenceUtil.getInstance(
	...
	public void onClick(View v) {
		...
		case R.id.layout_rzyx:
			if (isRzyx) {
				intentAction(CertificateActivity.this,
						UpdateEmailActivity.class);
				finish();
			} else {
				intentAction(CertificateActivity.this,
						BundlingEmailActivity.class);
				finish();
			}
			break;
		}
